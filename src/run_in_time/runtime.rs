...
use crate::run_in_time::environment::*;
use RuntimeValType::*;
    ...
    match stmt_node.e_type {
        ExpressionType::BinaryExpression {
            ...
            match (lhs.rt_type, rhs.rt_type) {
                (IntRT(l), IntRT(r)) => {
                    Ok(eval_int_numeric_binary(l, r, operator.value.unwrap()).unwrap())
                }
                (FloatRT(l), FloatRT(r)) => {
                    Ok(eval_float_numeric_binary(l, r, operator.value.unwrap()).unwrap())
                }
